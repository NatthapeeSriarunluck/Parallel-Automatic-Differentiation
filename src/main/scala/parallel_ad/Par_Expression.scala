...
case class Sum(e1: Par_Expression, e2: Par_Expression) extends Par_Expression {
  ...
  override def derive(seed: Double, varAssn: Map[String, Double]): Unit = {
    e1.value = Par_Process.eval(e1, varAssn)
    e2.value = Par_Process.eval(e2, varAssn)
...
case class Power(b: Par_Expression, e: Par_Expression) extends Par_Expression {
  ...
  override def derive(seed: Double, varAssn: Map[String, Double] ): Unit = {
    b.value = Par_Process.eval(b, varAssn)
    e.value = Par_Process.eval(e, varAssn)
